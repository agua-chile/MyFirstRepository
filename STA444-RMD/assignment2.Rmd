...
myFill <- function( x )
{
  ...
  for( i in 1:length( x ) )
  {
    ifelse( !is.na( x[i] ), mostRecent <- x[i], x[i] <- mostRecent )
  }
  return( x )
} 
```
...
boot.lm <- function( model, N=1000 )
{
  ...
  for( i in 1:N )
  {
    data_sample <- data %>% sample_frac( replace=TRUE )
    model.boot <- lm( formula, data=data_sample )
...
#' @param model
#' @param N
genHist <- function( v, title="Title", xName="X", yName="Y", caption="Caption", subtitle="Subtitle", tag="Tag", altInsight="Alt Insight", label="Label" )
{
  data <- data.frame( x=v )
  ggplot( data=data ) + 
    geom_histogram( aes( x=v ) ) +
    labs( title=title, x=xName, y=yName, caption=caption, subtitle=subtitle, tag=tag, alt_insight=altInsight, label=label )
}
genHist( rnorm( 10000 ) )
``` 
